...
def obtener_informacion_ip():
    ...
    try:
        ...
        puertos_y_servicios = []
        for port_info in resultado['data']:
            puerto = port_info['port']
            servicio = port_info['module']
            puertos_y_servicios.append(f"Port: {puerto} - Servicio: {servicio}")

        dominios_label.config(text="Nombres de dominio asociados a la IP:")
        dominios_info.config(text=', '.join(dominios))
        
        puertos_label.config(text="Puertos y Servicios:")
        puertos_info.config(text='\n'.join(puertos_y_servicios))
